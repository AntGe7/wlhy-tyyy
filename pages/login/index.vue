		...
		<view class="login-view">
			<view class="t-login">
				...
				<uv-tabs :list="list" @click="clickTab" lineWidth="80">
					<div v-for="(item) in list" :key="item.id" class="tab-item"
						:class="{ active: isPassword === item.id }" @click="clickTab(item.id)">
						{{ item.name }}
					</div>
				</uv-tabs>
							...
							<uv-form-item label="密码" prop="password" borderBottom>
								<uv-input v-model.trim="password" :type="isPass" placeholder="请输入密码" border="none">
									<template v-slot:suffix>
										<uv-icon style="height: 30rpx" :name="eyeIcon" @click="isEye"
											size="22"></uv-icon>
									</template>
								</uv-input>
							</uv-form-item>
					...
					<view v-else="isPassword == 2" id="form2" key="form2">
						<uv-form labelPosition="top" ref="form2" labelWidth="120">
							<uv-form-item label="手机号" prop="mobile" borderBottom>
								<uv-input v-model.trim="mobile" type="number" maxlength="11" placeholder="请输入手机号"
									border="none">
								</uv-input>
							</uv-form-item>
							<uv-form-item label="验证码" prop="captcha" borderBottom>
								<uv-input v-model.trim="captcha" type="number" maxlength="6" placeholder="请输入验证码"
									border="none">
									<template v-slot:suffix>
...
const onLogin = () => {
	...
	try {
		if (isPassword.value === 1) {
			...
			//加密base64.encode
			const encodedPassword = base64.encode(password.value);
